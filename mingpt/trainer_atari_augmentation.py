...
from .utils_augmentation import sample
import atari_py
...
class Trainer:
    ...
    def train(self):
        ...
        if self.config.load_model:
            checkpoint = torch.load("./swinT_weight_MsPacman.pt")
        ...
        def run_epoch(split):
            ...
            correct_pred = {action:0 for action in range(0,self.action_size)}
            total_pred = {action:0 for action in range(0,self.action_size)}
            ...
            for it, (x, y, r, t) in pbar:  # x: state, y: action, r: rtg, t:timestep
                ...
                y = y.to(self.device)  # (16, 1, 1)  2~5

                total_loss = []  # 한번 iteration 당 6개 loss 값만 저장
                sum_of_loss = 0.0
                with torch.set_grad_enabled(is_train):
                    with torch.cuda.amp.autocast():
                        features = swin_model(x)  # feature: (16, 4)
                        count = 1  # TODO 추가
                        for feature in features:
                            logits = feature.unsqueeze(1)  # (16, 1, 4)
                            loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), y.reshape(-1))
                            # loss 의 weight 를 추가하여 증강용 loss는 비중을 줄이는것
                            loss = loss if (count==1 or count==4) else torch.mul(loss, 0.7)
                            count += 1
                    ...
                    # epoch_loss.append(float(sum(total_loss))/len(total_loss))  # loss.item()
                    epoch_loss.append(float(sum_of_loss) / len(total_loss))
                    final_loss = total_loss[0]
                    for i in total_loss[1:]:
                        final_loss = torch.add(final_loss, i)
                ...
                if is_train:
                    ...
                    self.train_loss = float(sum(epoch_loss) / len(epoch_loss))
                    self.elapsed_time = time.time() - start_time  # duration


            if not is_train:
                # losses = torch.stack(losses)
                test_loss = float(sum(epoch_loss) / len(epoch_loss)) #float(np.mean(losses))
                ...
                return float(sum(epoch_loss) / len(epoch_loss)), total_accuracy # test_loss, test_accuracy
            else:
                correct_count = 0
                total_count = 0
                for action_name, action_count in correct_pred.items():
        ...
        mode = 'a' if os.path.isfile('rewLog_{now}.txt') else 'w'
        with open(f'rewLog_{now}.txt', mode) as f:
            for s in scores:
                sen = str(s) + '\n'
                f.write(sen)
        f.close()
    ...
    def get_returns(self):  # 실제 모델을 돌려서 나온 reward(rtg) 계산
        ...
        for i in range(test_iteration):  # 10번을 돌려 -> 3으로 수정 (빨리 보려고)
            ...
            sampled_action = sample(self.swin_model.module, state, temperature=1.0, sample=True, top_k=None)
            ...
            while True:
                ...
                sampled_action = sample(self.swin_model.module, state,temperature=1.0, sample=True, top_k=None)  # all_states.unsqueeze(0)
        env.close()
...
class Env():
    def __init__(self, args):
        ...
        actions = self.ale.getLegalActionSet() # self.ale.getMinimalActionSet()
        # _actions = self.ale.getMinimalActionSet()
