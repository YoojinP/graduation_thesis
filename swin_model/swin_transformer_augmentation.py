...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, pad_mask= None):
        ...
        if mask is not None:
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0) + pad_mask.unsqueeze(1).unsqueeze(0)
        else:
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        pad_attn_mask = pad_attn_mask.masked_fill(pad_attn_mask != 0, float('-900')).masked_fill(pad_attn_mask == 0, float(0.0))  # -100.0 -> -inf
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                # x: (2, 14, 18, 32)
                # torch.roll 과정: 1. [[[ ]]] 안의 14개의 항목들[[ ]]을 하나씩 위로 이동, 2. [[ ]] 안의 18개의 항목들을 하나씩 앞으로 땡김, 첫번째꺼는 결과적으로 마지막 순서가됨
                # 가로 row를 위로 1씩 이동, 맨 위는 맨 아래로 -> 세로 열 1씩 이동, 맨 첫번째 [32] 가 [[ 18 ]] 맨뒤로 감
        ...
        # W-MSA/SW-MSA  (144, 9, 32)
        attn_windows = self.attn(x_windows, mask=self.attn_mask, pad_mask=self.pad_mask)  # nW*B, window_size*window_size, C

        # merge windows (144, 3, 3, 32)
        ...
        if self.shift_size > 0 and self.block_num == 2:
            ...
            shifted_x = window_reverse(attn_windows, self.window_size, H_, W)  # (4, 18, 18, 32)
            shifted_x = self.pad_del(shifted_x, H)
            x = shifted_x
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=(84,84), patch_size=(4,4), in_chans=1, num_classes=4,
        ...
        self.head2 = nn.Linear(32, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x, block_output = layer(x)
            block_features+= block_output

        # (1, 63, 128), (1, 252, 32)
        features = []
        for f in block_features:
            norm = self.norm if f.shape[2] >32 else self.norm2
            f = norm(f)  # B L C -> (2, 63, 128)
            f = self.avgpool(f.transpose(1, 2))  # B C 1 -> (30, 128, 1)
            f = torch.flatten(f, 1)  # (2, 128)
            features.append(f)
    ...
    def forward(self, x):  # (B =2, C=1, 254, 234)
        features = []
        swin_features = self.forward_features(x)
        for x in swin_features:
            head = self.head if x.shape[1] >32 else self.head2
            x = head(x)
            features.append(x)
        return features
